...
第一行语句表示生成10个服从标准正态分布$N(0, 1)$的随机数，并将其赋值为`x`，即储存为数据对象`x`。仅执行第一条语句并不能直接在 R 的控制台显示这一对象本身，需要像第二行那样输入对象名让 R 去调用这一对象，从而进行展示。由于是随机生成的，故读者在重复这一代码时会产生不同的结果。如想使结果具有可重复性，可使用`set.seed()`函数设定随机数种子数（seed），相同的种子数总会产生相同的结果，但这一结果*仅对*`set.seed()`**之后的第一条命令适用**。
...
使用`ls()`函数可查看**当前环境中的所有已赋值对象**。`ls`其实是英文 list（列出）的缩写。
...
- `<-`表示赋值
- 字符型向量中的元素应放在引号（`""`或`''`）中
- `T`与`F`是逻辑型数据`TRUE`与`FALSE`的缩写。多数情况下，`T`与`TRUE`、`F`与`FALSE`分别等价，故定义变量名时尽量不要使用这T和F这两个大写字母。从规范性出发，尽量不要用缩写表示逻辑型数据。
...
向量是 R 数据结构的基础。R 中的诸多函数都是向量化（vectorization）的，即对向量施加的运算会**作用于该向量的每一个元素**。前面命令中的`y <- x / 5`已经体现了向量化运算的实质。以下是其他示例。
...
- `pmax(x, y)`给出一个与所给向量中最长向量长度相等的向量，向量中的元素由相同位置所在向量的最大值组成，类似的还有pmin(),含义可自推。可尝试运行并修改以下内容,解释结果：
```{r}
...
数组是矩阵的拓展形式，同样只能存储同质数据，但**可有2个以上维度**。数组使用`array()`函数创建。对初学者而言，这种形式的数据结构较少遇见。
...
数据框是 R 里**应用最多**的数据结构，能存储不同类型的数据，可视为许多等长度（但类型可不相同）向量的组合，也可视为解除了存储数据类型限制的矩阵。数据框使用`data.frame()`函数创建。
...
`within()`命令的用法与`with()`类似，但可修改数据框，但应注意**保存操作结果**。
...
此外，在一些函数的具体操作中，要注意区分c(a,b,...)与list(a,b,...)的使用。理论上说，列表和向量虽均能存放多个元素，但向量中存储的为同一种元素，而列表中元素类型则可为多种类型。在日后使用上，注意函数要求是list()还是c()，及其区别。
...
“原子”的本意是“不可再分的基本微粒”，原子向量亦即最基本的向量和最基本的数据结构。前面提到，R 中最常见的基础数据类型包括数值型、 逻辑型和字符型整型。实际上，这是针对原子向量的而言的。准确地讲，R 中原子向量有 6 种基本类型：
...
前面所谓的数值型向量（numeric）其实包含整型、实数型和双精度型，通常只会遇到整型和双精度型两种形式。整型只存储整数而不包含小数，双精度型可存储`2e-308`至`2e+308`之间的实数，其中`e`表示科学计数法，`2e-308`表示 $2 \times 10^{-308}$，`2e+308`表示 $2 \times 10^{308}$。在计算机中，即使无限小数也有一定的限定精度，因此会出现如下情形
...
不妨这样理解：`typeof()`给出的类型（type），实质上是指数据在计算机中的储存形态，而 `mode()` 给出的类型（mode）则是一般意义上常人所理解的数据形态。

复数型和原味型的数据对象在一般的数据分析中很少用到，这里不加介绍。R 中所有对象的定义可参见其官方网页的说明：
...
R中的子集选取运算符（subsetting operators）主要包括`[ ]`、`[[ ]]`、`$`三种。三者功能相近又有区别。其中，`[ ]` 和 `[[ ]]` 相似，但是前者返回的是列表，后者返回的是列表中的内容；`$` 运算符与选定的字符串一起，实质上是 `[[ ]]` 的缩写。有时也分别把 `[ ]` 和`[[ ]]`简写为 `[` 和 `[[` 。

#### `[ ]` 和 `[[ ]]`

用 `[]` 提取元素时，输出与原数据相同的数据类型。若只想获取内容，需使用 `[[ ]]`。但`[[ ]]`必须与正整数或者字符串一起使用才能返回内容。例如
...
若用`[ ]`的方式提取第一列，可看到提取结果仍为一个数据框。

```{r}
head(mtcars[1])
```

用 `typeof()` 查看上述结果的数据类型，可有进一步的发现
...
可见，用 `[ ]` 选出的仍为 `list`；用 `[[ ]]` 的方式选取则变成了 `double` 型的变量，即原子向量。


#### `$` 运算符

`$` 是一个简写运算符，通常用来访问数据框中的变量， `mtcars$mpg` 等价于 `mtcars[["mpg", exact = FALSE]]`。与`[[ ]]` 之间的不同是，前者是部分匹配。
...
矩阵或者数组：抛弃所有长度为1的维度(下面这个例子中，不同用户产生的数据值不同，主要关注数据类型即可)。
```{r}
matrix(round(rnorm(16,10,3)), nrow = 4)
